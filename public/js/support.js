$(document).ready(function () {
    ...
    $('#list-1-value-3').on('click', function () {
        ...
        $('#model-pragraph').text("We're so sorry you're having trouble with your discount! ");
    });
    $('#list-1-value-4').on('click', function () {
        $('#model-heading').text('Driver took the wrong route');
        $('#model-pragraph').text("We're really sorry the driver took you the wrong way. However, we also want you to know that the price should not vary from what we showed you when you ordered the journey, as long as none of the following happened.")
    });
    $('#list-1-value-6').on('click', function () {
        $('#model-heading').text('I had an accident');
        $('#model-pragraph').text('We hope this will never happen to you but if you have had an accident, please make sure that everyone involved is safe and, if necessary, call the local authorities. You can share the details of the accident below and we will contact you as soon as possible.');
    });
    $('#list-1-value-5').on('click', function () {
        $('#model-heading').text("I cancelled and was charged");
        $('#model-pragraph').text("Bear in mind that sometimes there may be a reason why you could have been charged for a journey you didn’t make.");
    });
    ...
    $('#list-2-value-1').on('click', function () {
        ...
        $('#model-pragraph').text("If the vehicle in which you have traveled was in poor condition, please tell us. We aim to ensure that the vehicles we offer you are modern and in good condition.")
    });

    $('#list-2-value-2').on('click', function () {
        $('#model-heading').text("Poor or dangerous driving");
        $('#model-pragraph').text("If you think the driver drove dangerously during your journey, please tell us. Your safety is our priority and for this we put all possible technological means and select drivers who share our concern for this aspect.");
    });

    $('#list-2-value-3').on('click', function () {
        $('#model-heading').text("Innappropriate conduct");
        $('#model-pragraph').text("If you think the driver's behavior has been inappropriate, please tell us. We want to ensure you always have an excellent experience with us.");
    });

    $('#list-2-value-4').on('click', function () {
        $('#model-heading').text("Arrived late to pick up");
        $('#model-pragraph').text("If the driver arrives at your reservation late, due to traffic conditions or for any other reason, please contact us and we will evaluate how we can best compensate you. In addition, if the driver does not arrive at the reservation and you want to cancel it, we assume the error and if charged for the cancellation, you will be given a refund if you let us know what happened.");
    });

    $('#list-2-value-5').on('click', function () {
        $('#model-heading').text("Vehicle was unclean");
        $('#model-pragraph').text("If the vehicle you traveled in did not meet your expectations of cleanliness, please let us know. We want your ride to meet the highest quality standards.");
    });
    ...
    $('#list-3-value-1').on('click', function () {
        ...
        $('#model-pragraph').text("If the app seems to be working properly but you can’t order a journey, your account may have been temporarily suspended. Please tell us in this form and we will help you solve your problem as soon as possible.");
    });
    ...
    $('#list-3-value-7').on('click', function () {
        ...
        $('#model-pragraph').html("Select 'My info and change whatever details you want to.<ul><li>Select 'My Company' from the menu.</li><li>Review the information in the 'Company details' section.</li><li>And finally, save your changes.</li></ul>");
    });
