$(document).ready(function() {
  ...
  function init() {
    ...
    $(document).on("click", ".remove-route", function() {
      ...
      $(this)
        .nextAll()
        .data("waypt-index", $(this).data("waypt-index") - 1);
  ...
  function displayPlacesAroundMarker(marker) {
    ...
    markers.forEach(function(m) {
      if (m.type === "nearby") {
        var geourl =
          "http://api.geonames.org/findNearbyPlaceNameJSON?radius=50&lat=" +
          m.position.lat() +
          "&lng=" +
          m.position.lng() +
          "&cities=cities10000&username=tripstop";
        ...
        $.ajax({ url: geourl, method: "GET" }).done(function(geoResponse) {
          if (geoResponse.geonames.length > 0) {
            for (var i = 0; i < geoResponse.geonames.length; i++) {
              var nearbyPlace = $("<div>")
                .addClass("nearby-place-div")
                .text(geoResponse.geonames[i].name.substr(0, 19))
                .attr("data-lat", geoResponse.geonames[i].lat)
                .attr("data-lng", geoResponse.geonames[i].lng);
              nearbyPlace.append(
                '<span class="fa fa-bed fa-fw fa-action" style="font-size:18px"></span>'
              );
              nearbyPlace.append(
                '<span class="fa fa-cutlery fa-fw fa-action" style="font-size:18px"></span>'
              );
              nearbyPlace.append(
                '<span class="fa fa-thermometer-full fa-fw fa-action"  style="font-size:18px"></span>'
              );
            ...
            $("#msgModaltitle").html(
              '<span class="fa fa-exclamation-triangle" style="font-size:24px"></span> Sorry'
            );
            $("#modal-message").text(
              "There are not way points here.  Please try another location"
            );
            $("#msgModal").modal("show");
          }
  ...
  function createMarker(place) {
    ...
    google.maps.event.addListener(marker, "click", function() {
      ...
      for (var i = 0; i < waypts.length; i++) {
        if (
          waypts[i].location.lat === location.lat &&
          waypts[i].location.lng === location.lng
        ) {
          wayptExists = true;
          index = i;
          break;
      ...
      if (place.rating == null) {
        rating = $("<p>")
          ...
          .addClass("place-rating");
      } else {
        rating = $("<p>")
          .text("Rating: " + place.rating + " of 5")
          .addClass("place-rating");
      }
      ...
      if (wayptExists) {
        toggleRoute = $("<p>")
          ...
          .addClass("remove-route")
          .attr("data-lat", this.position.lat())
          .attr("data-lng", this.position.lng())
          .attr("data-waypt-index", index)
  ...
  function getDestlnglat(dest) {
    ...
    var settings = {
      url:
        ...
        deststr +
        "&components=locality&key=AIzaSyB4Bfs-GG2wm1xuIfsRFm7MOc8K9gcSr9M",
      method: "GET"
    };
  ...
  function getWeather(lat, lng, category, city) {
    var settings = {
      ...
      crossDomain: true,
      url:
        "https://api.darksky.net/forecast/21641b7b2b96f7eede5a22906c35deb8/" +
        lat +
        "," +
        lng +
        "?exclude=flags%2Cminutely%2Chourly",
      method: "GET",
      dataType: "jsonp"
    };
    ...
    $.ajax(settings).done(function(response) {
      if (response == "Not Found") {
        $("#msgModaltitle").html(
          '<span class="fa fa-exclamation-triangle" style="font-size:24px"></span> Oops'
        );
        $("#modal-message").text(
          "Sorry, weather data is not available right now."
        );
        $("#msgModal").modal("show");
      } else {
        if (category == "destination") {
          for (i = 0; i < response.daily.data.length; i++) {
            weatherdate = weatherdate = moment()
              .add(i, "d")
              .format("MM/DD");
          ...
          for (i = 0; i < response.daily.data.length; i++) {
            ...
            lowtemp = response.daily.data[i].temperatureMin;
            weatherforecast = response.daily.data[i].summary;
            $(".table-weather-modal > tbody").append(
              "<tr><td>" +
                weatherdate +
                "</td><td>" +
                hightemp +
  ...
  function makeShort() {
    ...
    xhr.open(
      "GET",
      "https://api-ssl.bitly.com/v3/shorten?access_token=6f2e76e0fdc2ee6a57cfe862e16e1c19909c4efd&longUrl=" +
        encodeURI(dirurl)
    );
